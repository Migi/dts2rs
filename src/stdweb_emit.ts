...
function getInArgPosName(type: data.Type, curNS: data.Namespace) : string {
	switch (type.kind) {
		...
		case "optional": {
			if (type.subtype.canBeUndefined) {
				return getStructName(type.subtype, curNS);
			} else {
				return "Option<"+getStructName(type.subtype, curNS)+">";
			}
		}
...
function getStructName(type: data.Type, curNS: data.Namespace) : string {
	switch (type.kind) {
	...
	switch (type.kind) {
		...
		case "optional": {
			if (type.subtype.canBeUndefined) {
				...
				return "match "+s+" { ::stdweb::Value::Undefined => None, ___other => Some("+constructTypeFromJsValue(type, curNS, "___other")+") }";
			} else {
				// in the regular case I will let null map to None though.
				return "match "+s+" { ::stdweb::Value::Undefined | ::stdweb::Value::Null => None, ___other => Some("+constructTypeFromJsValue(type, curNS, "___other")+") }";
			}
		}
		case "class": return curNS.getRustPathTo(type.namespace, type.rustName);
		case "interface": return curNS.getRustPathTo(type.namespace, type.rustName);
...
class StdwebEmitter {
	...
	emitDocs(docs: string, writeln: (s:string) => void) : void {
		if (docs == "") {
			return;
		}
	...
	emitClass(writeln: (s:string) => void, theClass: data.ClassType) : void {
		let subClassOfTrait = getSubClassOfTraitName(theClass);

		this.emitDocs(theClass.docs, writeln);
		...
		theClass.methods.forEachResolvedFunction((method) => {
			if (!dontOutputThese.has(method.f.unresolvedRustName)) {
				this.emitFunction(util.indentAdder(writeln), method, FunctionKind.METHOD, true, theClass.namespace);
			}
		});
		theClass.properties.forEach((prop) => {
			if (!dontOutputThese.has("get_"+prop.rustName)) {
				writeln("\tfn get_"+prop.rustName+"(&self) -> "+getStructName(prop.type, theClass.namespace)+" {");
			...
			if (!dontOutputThese.has("set_"+prop.rustName)) {
				writeln("\tfn set_"+prop.rustName+"(&self, "+prop.rustName+": "+getInArgPosName(prop.type, theClass.namespace)+") -> &Self {");
		...
		this.emitRefTypeTraits(theClass.rustName, writeln);
		...
		theClass.constructors.forEachResolvedFunction((constructor) => {
			this.emitFunction(util.indentAdder(writeln), constructor, FunctionKind.CONSTRUCTOR, true, theClass.namespace);
		});
		theClass.staticMethods.forEachResolvedFunction((staticMethod) => {
			this.emitFunction(util.indentAdder(writeln), staticMethod, FunctionKind.CONSTRUCTOR, true, theClass.namespace);
		});
		writeln("}");
		writeln("");
		this.emitRefTypeTraits("__"+theClass.rustName+"_Prototype", writeln);
	}

	emitInterface(writeln: (s:string) => void, theInterface: data.InterfaceType) : void {
		let implementsTrait = getImplementsTraitName(theInterface);

		this.emitDocs(theInterface.docs, writeln);
		...
		theInterface.methods.forEachResolvedFunction((method) => {
			if (!dontOutputThese.has(method.f.unresolvedRustName)) {
				this.emitFunction(util.indentAdder(writeln), method, FunctionKind.METHOD, true, theInterface.namespace);
			}
		})
		theInterface.properties.forEach((prop) => {
			if (!dontOutputThese.has("get_"+prop.rustName)) {
				writeln("\tfn get_"+prop.rustName+"(&self) -> "+getStructName(prop.type, theInterface.namespace)+" {");
				writeln("\t\t"+constructTypeFromJsValue(prop.type, theInterface.namespace, "js!(return @{self}."+prop.jsName+";)"));
				writeln("\t}");
			}
			if (!dontOutputThese.has("set_"+prop.rustName)) {
				writeln("\tfn set_"+prop.rustName+"(&self, "+prop.rustName+": "+getInArgPosName(prop.type, theInterface.namespace)+") -> &Self {");
				writeln("\t\tjs!(@(no_return) @{self}."+prop.jsName+" = @{"+prop.rustName+"};);");
				writeln("\t\tself");
				writeln("\t}");
			}
		...
		this.emitRefTypeTraits(theInterface.rustName, writeln);
		...
		allProps.forEach((prop) => {
			if (!prop.isOptional) {
				defLine += ", "+prop.rustName+": "+getInArgPosName(prop.type, theInterface.namespace);
			}
		});
	...
	emitFunction(writeln: (s:string) => void, resolvedFunction: data.NameResolvedFunction, kind: FunctionKind, withImplementation: boolean, namespace: data.Namespace) {
		...
		if (shouldPrintDocs) {
			writeln("/**");
			let shouldPrintParamsAndReturn = false;
			if (theFunction.docs != "") {
		...
		}
		{
			...
			theFunction.signature.args.forEach((arg) => {
				...
				isFirstParam = false;
				line += arg.rustName + ": " + getInArgPosName(arg.type, namespace);
			});
			line += ") -> "+getStructName(theFunction.signature.returnType, namespace);
	...
	emitNamespace(writeln: (s:string) => void, theNamespace: data.Namespace) {
		...
		this.emitImplJsSerializeForType(eagerNamespaceRustName, "self.__js_ref", writeln);
		...
		if (theNamespace.parent != undefined) {
			theNamespace.staticFunctions.forEachResolvedFunction((f) => {
				this.emitFunction(util.indentAdder(writeln), f, FunctionKind.NAMESPACED_FUNCTION, true, theNamespace);
			});
		}
		writeln("}");
		writeln("");
		
		this.emitRefTypeTraits(lazyNamespaceRustName, writeln);

		if (theNamespace.parent == undefined) {
			theNamespace.staticFunctions.forEachResolvedFunction((f) => {
				this.emitFunction(writeln, f, FunctionKind.FREE_FUNCTION, true, theNamespace);
			});
		}
	}
	
	emitPreludeForNamespace(writeln: (s:string) => void, theNamespace: data.Namespace) {
		util.forEachKeyValueInObject(theNamespace.classes, (className, theClass) => {
		...
		util.forEachKeyValueInObject(theNamespace.subNamespaces, (subNsName, subNS) => {
			this.emitPreludeForNamespace(writeln, subNS);
		});
	}

	emitCargoToml(packageName: string) : string {
		return "" +
		"[package]\n" +
		"name = \""+packageName+"\"\n" +
		"version = \"1.0.0\"\n" +
	...
	emitLibRs(writeln: (s:string) => void, program: data.Program) {
		...
		writeln("");
		this.emitRefTypeTraits("__UntypedJsFn", writeln);
		...
		closures.forEach((f) => {
			...
			writeln("");
			this.emitRefTypeTraits(fStructName, writeln);
			...
			for (let i = 0; i < f.args.length; i++) {
				fnDefLine += ", arg"+(i+1)+": "+getInArgPosName(f.args[i].type, rootNameSpace);
			}
			fnDefLine += ") -> "+getStructName(f.returnType, rootNameSpace)+" {";
			writeln(fnDefLine);
			...
			let paramsStr = "";
			for (let i = 0; i < f.args.length; i++) {
				let argStr = getInArgPosName(f.args[i].type, rootNameSpace);
		...
		this.emitNamespace(writeln, rootNameSpace);

		writeln("pub mod prelude {");
		writeln("\tpub use Any;");
		writeln("\tpub use AsAny;");
		writeln("\tpub use FnHandle;");
		this.emitPreludeForNamespace(util.indentAdder(writeln), rootNameSpace);
		closures.forEach((f) => {
			let fTraitName = underscoreEscapeSignature("JsCallable", f);
